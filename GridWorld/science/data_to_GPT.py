...
def create_system_message_with_uncertainty(grid_height, grid_width):
    """
    Create a structured message for the 'system' containing instructions on how to process trajectory feedback.
    ...
    The "content" for the system includes descriptions of how to handle different types of feedback: 
    imperative, evaluative, and descriptive, and what to do with these feedback types in terms of mapping 
    to locations,labeling them appropriately, and providing an uncertainty quantification per location.
    """
    example_feedback = '''
                {"feedback": "it should not go below the bed", 
                "landmarks": {"clock": ["2f"], "bed": ["2c", "2d"]}, 
                "trajectory": ["1b", "1c", "1d", "1e", "1f", "1g", "2g", "3g", "4g", "4h"]}
                '''
    example_output = {'locations': ['1b', '1c', '1d', '1e'], 'label': 'NEG', 'feedback_type': 'imperative', 'certainty_locations': [0.8, 1, 1, 0.8]}
    ...
    example_feedback2 = '''
                ...
                "trajectory": ["1b", "1c", "1d", "1e", "1f", "1g", "2g", "3g", "4g", "4h"]}
                '''
    example_output2 = {'locations': ['3g', '4g', '4h'], 'label': 'POS', 'feedback_type': 'evaluative', 'certainty_locations': [0.6, 0.8, 1]}
    ...
    example_feedback3 = '''
                ...
                "trajectory": ["1b", "1c", "1d", "1e", "1f", "1g", "2g", "3g", "4g", "4h"]}
                '''
    example_output3 = {'locations': ['2f'], 'label': 'POS', 'feedback_type': 'imperative', 'certainty_locations': [1]}
    
    system_prompt = f"""
    An agent is trying to learn and follow a specific path in a {grid_height}x{grid_width} grid map.
    Your job is to translate the feedback of the current trajectory into feedback types, locations in the map with their corresponding certainty, and a label.

    - If the feedback type is imperative, compute what locations in the map the instructions are referring to and label them as either 'good' (go to) or 'bad' (avoid).

    - If the feedback type is evaluative, determine what locations in the map the feedback is evaluating and whether the feedback is positive or negative.
...
def create_system_message_immediate(grid_height, grid_width):
    example_feedback = '''
                {"feedback": "it should not go below the bed", 
                "landmarks": {"clock": ["2f"], "bed": ["2c", "2d"]}, 
                "trajectory": ["1b", "1c", "1d", "1e", "1f", "1g", "2g", "3g", "4g", "5g"]}
                '''
    example_output = {'immediate_locations': ['1c', '1d'],'surrounding_locations': ['1b', '1e'], 'label': 'NEG', 'feedback_type': 'imperative'}
    """
    Create a structured message for the 'system' containing instructions on how to process trajectory feedback.
...
def get_reward_function_structure_with_uncertainty(grid_width, grid_height):
    """
            ...
            "properties": {
                "locations": {
                    ...
                    "description": ("Locations in the grid referring to feedback with row numbers and a lowercase letter "
                                ...
                                "as you move upward), and columns are labeled from left to right (a is the leftmost). "
                ...
                "certainty_locations": {
                    ...
                    "description": "Certainty values for each location in the 'locations' array, ranging from 0 (completely uncertain) to 1 (completely certain)."
                }
            },
            "required": ["locations", "label", "certainty_locations"]
        }
    }
...
def get_reward_function_structure_immediate(grid_width, grid_height):
    """
    Generate a dictionary describing the expect structure for a 'getReward' function.
    ...
    Returns:
      dict: A dictionary that defines the structure of the 'getReward' function, detailing the
            function name, parameters, and their associated properties such as type and description.
    """
    loc_pattern = create_location_pattern(grid_width, grid_height) # Pattern ensures the locations match the grid format and stay within grid limits.
    return {
        "name": "getReward",
        "parameters": {
            "type": "object",
            "properties": {
                "immediate_locations": {
                    ...
                    "description": ("The immediate locations referred to by the feedback. "
                                    "The rows are numbered from bottom to top (1 is the lowest row, increasing "
                                    "as you move upward), and columns are labeled from left to right (a is the leftmost). "
                                    "For example, '1b' refers to the lowest row in the second column from the left.")},
                "surrounding_locations": {
                    ...
                    "description": ("Surrounding locations that could be referred to by the feedback. "
                                    "The rows are numbered from bottom to top (1 is the lowest row, increasing "
                                    "as you move upward), and columns are labeled from left to right (a is the leftmost). "
                                    "For example, '1b' refers to the lowest row in the second column from the left.")},
                "label": {
            ...
            "required": ["immediate_locations", "label", "certainty_locations"]
        }
    }
